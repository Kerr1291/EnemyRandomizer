...
namespace EnemyRandomizerMod
{
    ...
    public class EnemyRandomizerLogic
    {
        ...
        SmartRoutine randomEnemyLocator = new SmartRoutine();
        IEnumerator randomizerReplacer = null;

        SmartRoutine replacementController = new SmartRoutine();
            ...
            while( true )
            {
                ...
                if( UnityEngine.Input.GetKeyDown( KeyCode.O ) )
                {
                    for( int i = 0; i < UnityEngine.SceneManagement.SceneManager.sceneCount; ++i )
                    {
                        ...
                        if( status )
                        {
                            string outputPath = Application.dataPath + "/Managed/Mods/" + s.name;
                            Dev.Log( "Dumping Scene to " + outputPath);
        ...
        public void Setup( bool simulateReplacement )
        {
            ...
            if( RUN_DEBUG_INPUT )
            {
                ...
                SmartRoutineHelper.Instance.StartCoroutine( debugInput );
            }
        ...
        public void Unload()
        {
            ...
            if( RUN_DEBUG_INPUT )
            {
                SmartRoutineHelper.Instance.StopCoroutine( debugInput );
                debugInput = null;
            }

            if( randomEnemyLocator != null )
                randomEnemyLocator.Reset();
            randomEnemyLocator = new SmartRoutine();
        ...
        public void StartRandomEnemyLocator( Scene from, Scene to )
        {
            ...
            replacementController.UpdateBehavior = ControlReplacementRoot;
            ...
            randomEnemyLocator.UpdateBehavior = LocateAndRandomizeEnemies;
        ...
        IEnumerator LocateAndRandomizeEnemies(params object[] args)
        {
            for (; ; )
            {
            ...
            if(newEnemy.name.Contains( "Mega Zombie Beam Miner" ) ) 
            {
                //fix the targetting source?
                {
                    ...
                    foreach( var a in actions )
                    {
                        ...
                        {
                            GameObject beamOrigin = newEnemy.FindGameObjectInChildrenWithName("Beam Origin");
        ...
        void PositionRandomizedEnemy( GameObject newEnemy, GameObject oldEnemy, Vector3? startingPos = null )
        {
            ...
            //    ( SmartRoutineManager.Instance ).StartCoroutine( PositionCorrector( newEnemy, pos ) );
            //}

            //TODO: needs more work to figure out why it despawns? or teleports sometimes
            //if( newEnemy.name.Contains( "Giant Fly Col" ) )
            //{
            //    ( SmartRoutineManager.Instance ).StartCoroutine( PositionCorrector( newEnemy, oldEnemy.transform.position ) );
            //}
            ...
            if( ( flags & FLAGS.GROUND ) > 0 )
            {
                ...
                //{
                //    ( SmartRoutineManager.Instance ).StartCoroutine( PositionCorrector( newEnemy, newEnemy.transform.position + positionOffset ) );
                //}
            }
            ...
            if( newEnemyIsMantisFlyerChild ||( flags & FLAGS.WALL ) > 0 || ( flags & FLAGS.CRAWLER ) > 0 )
            {
                ...
                if( ( flags & FLAGS.CRAWLER ) > 0 )
                {
                    ...
                    //    //( SmartRoutineManager.Instance ).StartCoroutine( PositionCorrectorProjection( newEnemy, newEnemy.transform.rotation, upOffset, finalDir, 10f, 1f, 10f ) );
                    //}
        ...
        IEnumerator ControlReplacementRoot(params object[] args)
        {
            for (; ; )
            {
