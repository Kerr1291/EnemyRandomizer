...
namespace EnemyRandomizerMod
{
    public class HornetBoss : EnemySM
    {
        ...
        {
            if( gameObject.FindGameObjectInChildrenWithName( "Refight Range" ) != null )
                refightRange = gameObject.FindGameObjectInChildrenWithName( "Refight Range" ).AddComponent<RangeCheck>();
            if( gameObject.FindGameObjectInChildrenWithName( "Evade Range" ) != null )
                evadeRange = gameObject.FindGameObjectInChildrenWithName( "Evade Range" ).AddComponent<EvadeRange>();
            if( gameObject.FindGameObjectInChildrenWithName( "Sphere Range" ) != null )
                sphereRange = gameObject.FindGameObjectInChildrenWithName( "Sphere Range" ).AddComponent<RangeCheck>();
            if( gameObject.FindGameObjectInChildrenWithName( "A Sphere Range" ) != null )
                aSphereRange = gameObject.FindGameObjectInChildrenWithName( "A Sphere Range" ).AddComponent<RangeCheck>();
            if( gameObject.FindGameObjectInChildrenWithName( "A Dash Range" ) != null )
                aDashRange = gameObject.FindGameObjectInChildrenWithName( "A Dash Range" ).AddComponent<RangeCheck>();
            if( gameObject.FindGameObjectInChildrenWithName( "Hit ADash" ) != null )
                hitADash = gameObject.FindGameObjectInChildrenWithName( "Hit ADash" ).GetComponent<PolygonCollider2D>();
            if( gameObject.FindGameObjectInChildrenWithName( "Hit GDash" ) != null )
                hitGDash = gameObject.FindGameObjectInChildrenWithName( "Hit GDash" ).GetComponent<PolygonCollider2D>();
            if( gameObject.FindGameObjectInChildrenWithName( "Dust HardLand" ) != null )
                dustHardLand = gameObject.FindGameObjectInChildrenWithName( "Dust HardLand" ).GetComponent<ParticleSystem>();
            if( gameObject.FindGameObjectInChildrenWithName( "Run Away Check" ) != null )
                runAwayCheck = gameObject.FindGameObjectInChildrenWithName( "Run Away Check" ).AddComponent<RangeCheck>();


            if( gameObject.FindGameObjectInChildrenWithName( "Throw Effect" ) != null )
                throwEffect = gameObject.FindGameObjectInChildrenWithName( "Throw Effect" ).AddComponent<ThrowEffect>();
            if( gameObject.FindGameObjectInChildrenWithName( "A Dash Effect" ) != null )
                aDashEffect = gameObject.FindGameObjectInChildrenWithName( "A Dash Effect" ).AddComponent<ADashEffect>();
            if( gameObject.FindGameObjectInChildrenWithName( "G Dash Effect" ) != null )
                gDashEffect = gameObject.FindGameObjectInChildrenWithName( "G Dash Effect" ).AddComponent<GDashEffect>();
            if( gameObject.FindGameObjectInChildrenWithName( "Sphere Ball" ) != null )
                sphereBall = gameObject.FindGameObjectInChildrenWithName( "Sphere Ball" ).AddComponent<SphereBall>();
            if( gameObject.FindGameObjectInChildrenWithName( "Flash Effect" ) != null )
                flashEffect = gameObject.FindGameObjectInChildrenWithName( "Flash Effect" ).AddComponent<FlashEffect>();
            ...
            if( UnityEngine.SceneManagement.SceneManager.GetSceneByName( "Fungus1_04_boss" ).FindGameObject( "Corpse Hornet 1(Clone)" ) != null )
            {
                ...
                hornetCorpse.PrintSceneHierarchyTree( true, hornetCorpse.name );
        ...
        protected override IEnumerator ExtractReferencesFromExternalSources()
        {
            ...
            if( item != null )
            {
                item.PrintSceneHierarchyTree( true, item.name );
