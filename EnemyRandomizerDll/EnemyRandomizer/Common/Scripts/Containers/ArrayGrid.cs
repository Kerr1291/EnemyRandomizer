...
namespace nv
{
    ...
    [System.Serializable]
    public class ArrayGrid<T> : IEnumerable, IEnumerable<T>, IList<T>, IList
    {
        List<T> data;
        ...
        public Vector2Int CenterPosition
        {
            get
            {
                return Vector2Int.FloorToInt(SizeRect.center);
            }
        }
        ...
        {
            Resize(other.Size);
            CopyArea(other, other.SizeRect, SizeRect);
        }
        ...
        {
            source_area = Mathnv.Clamp(source_area, source.SizeRect);
            dest_area = Mathnv.Clamp(dest_area, dest.SizeRect);
        ...
        //resize the array. if possible, old data will be preserved
        ...
        public IEnumerable<T> GetEmptyElements()
        {
            return data.Where(x => (EqualityComparer<T>.Default.Equals(x, default(T))));
        }

        public IEnumerable<T> GetNonEmptyElements()
        {
            return data.Where(x => (!EqualityComparer<T>.Default.Equals(x, default(T))));
        }

        public IEnumerable<T> GetElementsOfType<U>()
        {
            return data.Where(x => typeof(U).IsAssignableFrom(x.GetType()));
            //return data.Where(x => (EqualityComparer<T>.Default.Equals(x, type)));
        }

        public IEnumerable<T> GetMatchingElements(T element)
        {
            return data.Where(x => (EqualityComparer<T>.Default.Equals(x, element)));
        }

        public IEnumerable<T> GetElementsInMask(List<T> mask)
        {
            return data.Where(x => (mask.Contains(x))).ToList();
        }

        public IEnumerable<T> GetElementsNotInMask(List<T> mask)
        {
            return data.Where(x => (!mask.Contains(x)));
        }
        ...
        public List<Vector2Int> GetMatchingPositions(T dataToMatch)
        {
            List<Vector2Int> positions = new List<Vector2Int>();

            for(int i = 0; i < data.Count; ++i)
            {
                if(EqualityComparer<T>.Default.Equals(data[i], dataToMatch))
                {
                    positions.Add(GetPositionFromIndex(i));
                }
            }
        ...
        {
            Mathnv.Clamp(ref area, SizeRect);
        ...
        public List<T> GetMatchingElementsInArea(Rect area, T dataToMatch)
        {
            Mathnv.Clamp(ref area, SizeRect);
            ...
            for(int j = IterJStart(area); j < IterJEnd(area); ++j)
            {
                for(int i = IterIStart(area); i < IterIEnd(area); ++i)
                {
                    ...
                    if(EqualityComparer<T>.Default.Equals(this[i, j], dataToMatch))
                        elements.Add(this[i, j]);
                }
            }
        ...
        public List<Vector2Int> GetMatchingPositionsInArea(Rect area, T dataToMatch)
        {
            Mathnv.Clamp(ref area, SizeRect);
        ...
        public List<T> GetMatchingAdjacentElements(Vector2Int pos, bool search_diagonal, T type)
        {
            if(!IsValidPosition(pos))
                return null;
        ...
        public List<Vector2Int> GetMatchingAdjacentPositions(Vector2Int pos, bool search_diagonal, T type)
        {
            if(!IsValidPosition(pos))
                return null;
        ...
        public List<T> GetNotMatchingAdjacentElements(Vector2Int pos, bool search_diagonal, T type)
        {
            if(!IsValidPosition(pos))
                return null;
        ...
        public List<Vector2Int> GetNotMatchingAdjacentPositions(Vector2Int pos, bool search_diagonal, T type)
        {
            if(!IsValidPosition(pos))
                return null;
        ...
        {
            return GetMatchingAdjacentElements(pos, search_diagonal, default(T));
        }

        public List<Vector2Int> GetAdjacentEmptyPositions(Vector2Int pos, bool search_diagonal)
        {
            return GetMatchingAdjacentPositions(pos, search_diagonal, default(T));
        }
        ...
        public List<Vector2Int> GetMatchingPositionsOfAreas(T dataToMatch, Vector2Int desiredSize)
        {
            List<Vector2Int> areas = new List<Vector2Int>();
            ArrayGrid <int> goodPoints = new ArrayGrid<int>(Size, 0);

            for(int j = IterJStart(SizeRect); j < IterJEnd(SizeRect); ++j)
            {
                int horizontalCount = 0;
                for(int i = IterIStart(SizeRect); i < IterIEnd(SizeRect); ++i)
                {
                    if(!IsValidPosition(i, j))
                        continue;
